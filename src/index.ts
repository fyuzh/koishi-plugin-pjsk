...
const generate = async (ctx, session, options, inputText) => {
...
export function apply(ctx: Context) {
  ...
  ctx.on('message', async (session) => {
    if (session.content.endsWith('.jpg')) {
      await session.send(await generate(ctx, session, {}, session.content.substring(0, session.content.length - 4)));
    }
  })
  ...
  ctx.command('pjsk.绘制 [inputText:text]', '绘制表情包')
    ...
    .action(async ({session, options}, inputText) => {
      return await generate(ctx, session, options, inputText);
    })
